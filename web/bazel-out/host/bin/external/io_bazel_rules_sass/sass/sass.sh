...
export BAZEL_TARGET=@io_bazel_rules_sass//sass:sass
export BAZEL_WORKSPACE=project
if [[ -z "${BAZEL_NODE_MODULES_ROOT:-}" ]]; then
  export BAZEL_NODE_MODULES_ROOT=build_bazel_rules_sass_deps/node_modules
fi
...
export BAZEL_NODE_PATCH_REQUIRE=$(rlocation "io_bazel_rules_sass/sass/sass_require_patch.js")
...
MAIN=$(rlocation "io_bazel_rules_sass/sass/sass_loader.js")
...
for ARG in "${ALL_ARGS[@]:-}"; do
  case "$ARG" in
    ...
    --nobazel_patch_module_resolver)
      MAIN="external/io_bazel_rules_sass/sass/sass_wrapper"
      ...
      MODULES_MANIFEST=${MODULES_MANIFEST:-$(rlocation "io_bazel_rules_sass/sass/_sass.module_mappings.json")}
      ;;
...
if [[ ! -f "$MAIN" ]]; then
  MAIN=$(rlocation "io_bazel_rules_sass/sass/sass_wrapper")
fi
